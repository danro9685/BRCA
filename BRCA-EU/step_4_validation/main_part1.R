...
# process raw data
data = fread(input="ICGC/WGS/simple_somatic_mutation.open.tsv",sep="\t",header=TRUE)
...
# save the results
trinucleotides_counts_wgs = trinucleotides_counts
save(trinucleotides_counts_wgs,file="processed_data/trinucleotides_counts_wgs.RData")

# process raw data
data = fread(input="ICGC/WXS/simple_somatic_mutation.open.tsv",sep="\t",header=TRUE)
data = data[,c("icgc_donor_id","project_code","chromosome","chromosome_start","chromosome_end","assembly_version","mutation_type","mutated_from_allele","mutated_to_allele","platform","sequencing_strategy")]
data = data[which(data[,"chromosome"]!="MT"),]
data = data[which(data[,"chromosome_start"]==data[,"chromosome_end"]),]
data = data[which(data[,"assembly_version"]=="GRCh37"),]
...
data = data[which(as.matrix(data[,"mutated_to_allele"])%in%c("A","C","G","T")),]
data = data[which(as.matrix(data[,"platform"])%in%c("Illumina HiSeq","Illumina HiSeq X Ten","Illumina GA sequencing")),]
data = data[which(data[,"sequencing_strategy"]=="WXS"),]
...
# save the results
trinucleotides_counts_wxs = trinucleotides_counts
save(trinucleotides_counts_wxs,file="processed_data/trinucleotides_counts_wxs.RData")
